...
void DisplayTransactionID()
{
	CComPtr<IObjectContextInfo> spInfo;
	HRESULT hr = ::CoGetObjectContext(__uuidof(IObjectContextInfo), (void**) &spInfo);
	_ASSERT (SUCCEEDED(hr));

	CComPtr<IUnknown> spUnk;
	hr = spInfo->GetTransaction(&spUnk);
	_ASSERT (SUCCEEDED(hr));

	CComPtr<ITransaction> spTran;
	hr = spUnk->QueryInterface(&spTran);
	_ASSERT (SUCCEEDED(hr));

	XACTTRANSINFO info;
	hr = spTran->GetTransactionInfo(&info);
	// info.uow contains the raw transaction ID
