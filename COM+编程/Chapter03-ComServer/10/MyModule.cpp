...
HRESULT CMyModule::RegisterClassObject()
{
	CVcrClassObject* pVCRClassObject = new CVcrClassObject;

	if (NULL == pVCRClassObject) {
		return E_OUTOFMEMORY;
	}

	IUnknown* pUnk = pVCRClassObject->GetUnknown();
	HRESULT hRes = ::CoRegisterClassObject(CLSID_VCR,
		pUnk,
		CLSCTX_SERVER,
		REGCLS_MULTIPLEUSE,
		&m_dwVcr); 
	...
	if (FAILED(hRes)) {
		delete pVCRClassObject;
	}

	return hRes;
}

HRESULT CMyModule::RevokeClassObject()
{
	return ::CoRevokeClassObject(m_dwVcr);
}
...
HRESULT CMyModule::RegisterServer(void)
{
	...
	HRESULT hr = AddRegistryEntry(
		"CLSID\\{318B4AD3-06A7-11d3-9B58-0080C8E11F14}\\LocalServer32", "", szPath);
	return hr;
}
